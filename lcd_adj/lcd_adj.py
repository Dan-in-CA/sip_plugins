...
urls.extend(
    [
        "/lcd",
        "plugins.lcd_adj.settings",
        "/lcdj",
        "plugins.lcd_adj.settings_json",
        "/lcda",
        "plugins.lcd_adj.update",
    ]
)
...
class LCDSender(Thread):
    ...
    def alarm(self, name, **kw):
        datalcd = get_lcd_options()
        if (
            datalcd["use_lcd"] != "off" and not self.alarm_mode
        ):  # if LCD plugin is enabled
    ...
    def notify_station_scheduled(self, name, **kw):
        datalcd = get_lcd_options()
        if (
            datalcd["use_lcd"] != "off" and not self.schedule_mode
        ):  # if LCD plugin is enabled
