...
_startup_stop_event = threading.Event()
...
def on_restart():
    """Updated restart function with proper thread cleanup"""
...
def delayed_start():
    """Delayed startup function that can be interrupted"""
    print("MQTT: Waiting 5 seconds for other plugins to initialize...")

    if not _startup_stop_event.wait(5):
        # Only start if we weren't asked to stop
        if not _startup_stop_event.is_set():
            print("MQTT: Starting connection monitor...")
            start_connection_monitor()
    else:
        print("MQTT: Startup cancelled")
...
def start_connection_monitor():
    ...
    if _connection_thread is None or not _connection_thread.is_alive():
        ...
        _connection_thread = threading.Thread(target=connection_monitor, daemon=False)
...
def stop_all_threads():
    ...
    print("MQTT: Stopping all threads...")

    # Stop startup thread
    _startup_stop_event.set()
    if _startup_thread and _startup_thread.is_alive():
        _startup_thread.join(timeout=2)

    # Stop connection monitor
    stop_connection_monitor()

    print("MQTT: All threads stopped")


# Initialize the plugin with proper thread management
if mqtt is not None:
    import time
    import threading

    # Start in a separate thread to avoid blocking other plugins
    _startup_thread = threading.Thread(target=delayed_start, daemon=False)
