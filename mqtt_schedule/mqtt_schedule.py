...
urls.extend(
    [
        "/mr1-sp",
        "plugins.mqtt_schedule.settings",
        "/mr1-save",
        "plugins.mqtt_schedule.save_settings",
    ]
)
...
def on_message(client, msg):
    ...
    if type(cmd) is list:
        if len(cmd) < num_sta:
            print(
                "MQTT schedule, not enough stations specified, assuming first {} of {}".format(
                    len(cmd), num_sta
                )
            )
            ...
            print(
                "MQTT schedule, too many stations specified, truncating to {}".format(
                    num_sta
                )
            )
