...
urls.extend(
    [
        "/wa",
        "plugins.weather_adj.settings",
        "/wj",
        "plugins.weather_adj.settings_json",
        "/uwa",
        "plugins.weather_adj.update",
    ]
)
        ...
        if options["auto_delay"] != "off":
            ...
            if weather:
                ...
                if delay > 0:
                    ...
                    gv.sd["rdst"] = (
                        gv.now + gv.sd["rd"] * 3600 + 1
                    )  # +1 adds a smidge just so after a round trip the display hasn't already counted down by a minute.
                    stop_onrain()
                elif delay == 0:
...
class update(ProtectedPage):
    ...
    def GET(self):
        ...
        with open(
            "./data/weather_adj.json", "w"
        ) as f:  # write the rain delay configuration to json
            json.dump(qdict, f)
        weather_to_delay()
        raise web.seeother("/")
